...
import { elapsedSeconds, groupLabels } from '../utils/workoutUtils';
import {
  computeDoneSets,
  computeTotalVolume,
  setHasValues,
} from '../utils/workoutMath';
import { saveSession, evaluatePR } from '../../../services/workoutService';
...
// Container component: owns session state effects, save/discard flows and
// PR evaluation; all row/card UI is delegated to presentational components.
export default function WorkoutScreen({ navigation }) {
  ...
  const doneSets = computeDoneSets(workout.exercises);
  const totalVolume = computeTotalVolume(workout.exercises);
  ...
  // Done-toggle handler for a single set: un-completes instantly; on
  // completion evaluates PRs (deduped against this session's bests), fires
  // haptics and surfaces the PR toast.
  const handleToggleDone = async (item, set) => {
    if (set.completed) {
      lightImpact();
      dispatch({ type: 'UNCOMPLETE_SET', exerciseKey: item.key, setKey: set.key });
      return;
    }
    if (!setHasValues(set)) return;
  ...
  return (
    <View style={styles.container}>
          ...
          return (
            ...
            >
              ...
              {notesKey === item.key && (
                <ExerciseNotesEditor
                  noteDraft={noteDraft}
                  dirty={notesDirty(item)}
                  justSaved={notesJustSaved}
                  styles={styles}
                  colors={colors}
                  onChangePersonal={(personal) => setNoteDraft((d) => ({ ...d, personal }))}
                  onChangeTrainer={(trainer) => setNoteDraft((d) => ({ ...d, trainer }))}
                  onSave={() => saveExerciseNotes(item)}
                />
              )}
              ...
              <SuggestionBanner
                sugg={suggestions[item.key]}
                dismissed={dismissedTmPrompts.has(item.key)}
                styles={styles}
                colors={colors}
                onUse={() => applySuggestion(item)}
                onDismissTmPrompt={() =>
      ...
      <SwapSheet
        ...
        onSelectAlternative={(name) => doSwap(swapEx, name)}
        onAdHocPicker={() => setAdHocPickerVisible(true)}
      />
