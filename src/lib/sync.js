...
//     // PARKED: the legacy pull is lossy (drops RPE/notes, and can overwrite
//   // local full-fidelity rows with the redacted cloud copy). Disabled until
//   // the gated restore (Phase 4 of the sync rebuild) replaces it.
//   return { skipped: true, reason: 'disabled_pending_restore_rewrite' };
//   const settings = await getSyncSettings();
...
// src/lib/sync.js — COMPATIBILITY SHIM.
//
// The unified sync engine (syncEngine.js) now owns ALL queue processing and
// uploads. This file remains only because App.js, AuthContext and
// SyncSettingsScreen import from it — every export either delegates to the
// engine or is a safe no-op. The old implementations (degraded session
// payloads that could zero server-side data, the lossy pullFromCloud
// restore) are RETIRED — do not resurrect them. Phase 4 replaces
// pullFromCloud with the real gated restore; Phase 5 rebuilds the sync
// settings screen.
import NetInfo from '@react-native-community/netinfo';
import { getDb } from '../db/db';
import { processQueue, getEngineStatus, enqueueUpsert } from './syncEngine';
...
export async function initConnectivityListener() {
  const checkConnection = async () => {
    try {
      ...
      if (wasOffline && connectivityState.isConnected) {
        const settings = await getSyncSettings();
        if (settings.sync_mode === 'auto') await syncPending();
      }
    } catch (e) {
      console.error('[SYNC] Connectivity check failed:', e.message);
    }
...
// ── Sync settings (unchanged behavior) ──────────────────────────────────
export async function getSyncSettings() {
  const db = await getDb();
  const row = await db.getFirstAsync('SELECT * FROM sync_settings WHERE id = 1');
  return row || { sync_mode: 'auto', sync_enabled: 1, last_synced_at: null };
...
// ── Queue processing → delegated to the unified engine ──────────────────
// processQueue enforces the sync mode itself ('local' never runs; 'manual'
// only when explicitly triggered). This is what makes the old entry points
// (app launch, reconnect, Sync Now) feed the NEW engine instead of the
// retired one.
export async function syncPending(opts = {}) {
  return processQueue({ manual: !!opts.manual });
}

// RETIRED — the legacy lossy restore (dropped RPE/notes, exercise-id-1
// fallback, no gate, no retry). Phase 4 installs the real restore flow.
export async function pullFromCloud() {
  return { skipped: true, reason: 'restore_coming_in_phase_4' };
}

// ── Status views → the engine's truth ───────────────────────────────────
export async function getPendingSyncCount() {
...
export async function getSyncStatus() {
  const s = await getEngineStatus();
  let status = 'synced';
  if (!s.isConnected) status = s.pending_count > 0 ? 'offline-pending' : 'offline';
  else if (s.pending_count > 0) status = 'pending';
  return {
    status,
...
// Legacy bridge: any code still calling addToSyncQueue is routed to the
// engine's dedup-in-place enqueue.
export async function addToSyncQueue(entityType, entityId) {
  return enqueueUpsert(entityType, entityId);
}

// Legacy constants kept for import compatibility only.
