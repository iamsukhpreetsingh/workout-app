// nutritionTargetsCalc.js — CommonJS MIRROR of the mobile domain module
// (src/features/diet/domain/nutritionTargets.js). The backend target
// service must produce IDENTICAL recommendations to the client's live
// preview. Both copies are covered by test suites asserting the same
// scenarios — if one changes behavior, the other must too.
//
// Pipeline: profile → BMR (Mifflin-St Jeor) → activity factor →
// goal/intensity adjustment → daily calorie target → macro distribution.
// The UI shows ONLY the final numbers — formulas stay internal.
...
function validateNutritionProfile(p = {}) {
  const missing = [];
  const errors = [];
...
function validateProvidedNutritionFields(patch = {}) {
  const errors = [];
  const num = (v) => (v === '' || v == null ? null : Number(v));
